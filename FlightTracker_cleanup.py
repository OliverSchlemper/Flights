...
class FlightTracker:
    ...
    def __init__(self, start_time, stop_time=None, destination='./flights/flights.db', already_calculated=False, R = 150):
        #make dirs
        ...
        self.R = R
        self.destination_flights_db = destination
        self._init_stations()


        if already_calculated == False:
            self.download_and_process_db_files() # downloads flight tracker data and saves a db file with flights / flights_distinct
    ...
    #-------------------------------------------------------------------------------------------------------------------
    #-------------------------------------------------------------------------------------------------------------------
    def _append_enu(dataframe, lon0=-38.45, lat0=72.58, z0=0):
        """
    ...
    def _init_stations(self):
        ...
        self.stations =  FlightTracker._append_enu(stations, lon0 = stations.longitude.mean(), lat0 = stations.latitude.mean())

    #-------------------------------------------------------------------------------------------------------------------
    def process_db_files(filedir='./data/',
                        tablename='flights',
        ...
        for filename in files:
            ...
            if len(table_name_aircraft) == 0:
                ...
                df = pd.read_sql_query(f"SELECT *, date(readtime) as date from aircraft Where readtime >= '{self.start_time}' And readtime < '{self.stop_time}'", con)
        ...
        if len(flights) != 0:   
            ...
            flights = FlightTracker._append_enu(flights)
            ...
            # Filter data for radial distance less than R (60 km)
            flights = flights[flights.r2 < self.R**2]
            # Distinct flights
        ...
        con = sqlite3.connect(self.destination_flights_db)
    ...
    #-------------------------------------------------------------------------------------------------------------------
    def download_and_process_db_files(self):
        # get and unzip files
        ...
        FlightTracker.process_db_files(datetime.strftime(start_time.astimezone(FlightTracker.london), FlightTracker.fmt), datetime.strftime(stop_time.astimezone(FlightTracker.london), FlightTracker.fmt), destination = destination, tablename = 'flights', R = R)
        ...
        for i in range(3):
            for j in range(3):
                ...
                if i > 0 or j > 1:
                    ...
                    temp_f = FlightTracker._append_enu(self.f, self.stations[self.stations['Station Nr.'] == nr]['longitude'].to_numpy()[0], self.stations[self.stations['Station Nr.'] == nr]['latitude'].to_numpy()[0])
